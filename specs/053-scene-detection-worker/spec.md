...
1. **Scene Detection Engine Abstraction**
   ...
   - Runtime implementation: `PySceneDetectAdapter` using PySceneDetect library for real scene detection
   - Engine selection configurable via environment variable `SCENE_DETECTION_ENGINE` (default: `pyscenedetect`)
   - `opencv-python-headless` where GUI is unnecessary (PySceneDetect dependency)
...
- `PySceneDetectAdapter` uses PySceneDetect library
- Configuration via environment variables
- Uses ContentDetector (default threshold) for content-aware scene detection
...
|---------------------|---------|-------------|
| `SCENE_DETECTION_ENGINE` | `pyscenedetect` | Engine selection (`deterministic` for CI, `pyscenedetect` for production) |
| `SCENE_DETECT_THRESHOLD` | `27.0` | ContentDetector threshold |
| `SCENE_DETECT_TIMEOUT_SECONDS` | `120` | Timeout for scene detection |

### CI Behavior

- `SCENE_DETECTION_ENGINE=deterministic` in CI
- No model downloads
...
- `markCompleted(string $detector, string $detectorVersion, array $parameters, array $scenes)`: detecting → completed
...
10. PySceneDetect runtime adapter implemented and configurable
11. ProcessMediaAction has `detectScenes()` method
12. MediaSceneAnalysis model exists with correct schema
13. MediaSceneAnalysis migration creates `media_scene_analyses` table
14. MediaAsset has `sceneAnalysis()` relationship
15. ProcessMediaAsset pipeline restructured for independent scene detection
16. ProcessMediaAsset creates MediaSceneAnalysis on video assets
17. ProcessMediaAsset is idempotent (skips if MediaSceneAnalysis exists)
18. ProcessMediaAsset marks scene detection failed on error
19. Scene detection failure does NOT block audio extraction/transcription
20. Transcription failure does NOT block scene detection
21. Video without audio still receives scene detection
22. MediaSceneAnalysis lifecycle follows pending → detecting → completed/failed
23. Scene format is JSON array of {index, start_ms, end_ms}
24. Scene invariants enforced (ordered, non-overlapping, valid timing)
25. Empty scenes return empty array (not error)
26. Deterministic unit tests exist for Python worker scene detection logic
27. Deterministic unit tests exist for worker CLI detect-scenes command
28. Deterministic feature tests exist for MediaSceneAnalysis model
29. Deterministic feature tests exist for ProcessMediaAsset scene detection
30. All tests deterministic, no model downloads, no skips
31. All existing tests pass without modification
32. Code follows project conventions (Pint for PHP, flake8 for Python)
33. No secrets in logs or worker payloads
...
2. **External Dependencies**:
   - PySceneDetect Python library (runtime only)
   - opencv-python-headless (runtime only, PySceneDetect dependency)
   - Python 3.10+ runtime
